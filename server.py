...
# ==========================================
# 🛠️ TOOL 1: BOOKING TOOL
# ==========================================
def book_appointment_logic(tool_input: str):
    """
    Accepts a single string with booking details.
    """
    print(f"🚀 TRIGGERING API: Processing booking request: {tool_input}")
    return f"SUCCESS: Appointment booked! Details: {tool_input}. Ticket ID: #998877."

booking_tool = Tool(
    name="book_appointment_api",
    func=book_appointment_logic,
    description="Use this tool to book appointments. Input the full details (Name, Date, Reason) as a single text string."
)
...
async def create_organization(org: OrgCreate):
    clean_name = org.name.lower().replace(" ", "_")
    org_id = f"{clean_name}_{uuid.uuid4().hex[:8]}"
    return {"organization_id": org_id, "message": "Created"}
...
async def upload_document(organization_id: str, file: UploadFile = File(...)):
    ...
    return {"message": "Document processed", "chunks_added": len(splits)}

# --- ENDPOINT 3: CHAT ---
@app.post("/chat")
async def chat(request: ChatRequest):
    # 1. Setup Vector Store for this specific company
    vectorstore = Chroma(
        persist_directory=DB_FOLDER, 
        embedding_function=embeddings,
        collection_name=request.organization_id
    ...
    # 2. DEFINING THE RAG TOOL MANUALLY (The Fix)
    # We define this function INSIDE the endpoint so it can access 'vectorstore'
    def retrieve_documents(query: str) -> str:
        ...
        # Manually join the text to avoid 'missing key' errors
    ...
    rag_tool = Tool(
        ...
        description="Use this tool to search for information in company documents/policies."
    )

    # 3. Combine Tools
    tools = [rag_tool, booking_tool]

    # 4. Setup LLM (Temperature 0 is vital for agents)
    llm = ChatOllama(model=CHAT_MODEL, temperature=0)

    # 5. Initialize Agent
    # We use ZERO_SHOT_REACT_DESCRIPTION which works best with simple Tools
    ...
    # Prepend history to the input so the Agent sees context
    full_query = f"""
    ...
    # 7. Run Agent
    print(f"🤖 Agent Thinking for {request.session_id}...")
